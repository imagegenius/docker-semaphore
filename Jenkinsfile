pipeline {
  ...
  environment {
    ...
    CI_PORT = '3000'
    CI_SSL = 'false'
    CI_DOCKERENV = 'SEMAPHORE_DB_DIALECT=bolt|SEMAPHORE_ADMIN_PASSWORD=password|SEMAPHORE_ADMIN_NAME=admin|SEMAPHORE_ADMIN_EMAIL=admin@localhost|SEMAPHORE_ADMIN=admin|SEMAPHORE_ACCESS_KEY_ENCRYPTION=supersecretkey'
    CI_AUTH = ''
    CI_WEBPATH = ''
  }
  stages {
    ...
    stage('Build-Single') {
      ...
      steps {
        ...
        sh "docker buildx build \
          ...
          --label \"org.opencontainers.image.url=https://github.com/imagegenius/docker-semaphore/packages\" \
          --label \"org.opencontainers.image.source=https://github.com/imagegenius/docker-semaphore\" \
          ...
          --label \"org.opencontainers.image.description=Semaphore is a modern UI for Ansible. It lets you easily run Ansible playbooks, get notifications about fails, control access to deployment system.\" \
